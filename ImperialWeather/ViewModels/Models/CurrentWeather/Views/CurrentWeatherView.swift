        ...
        VStack(spacing: 12) {
            ...
            VStack(spacing: 8) {
                Text(temperatureScale == .celsius
                    ? viewModel.currentWeather.conditions.temperature.celsiusStringShort
                    : viewModel.currentWeather.conditions.temperature.fahrenheitStringShort
                )
                .font(.largeTitle)
                .fontWeight(.bold)
                .accessibility(label:
                    Text(temperatureScale == .celsius
                        ? viewModel.currentWeather.conditions.temperature.celsiusString
                        : viewModel.currentWeather.conditions.temperature.fahrenheitString)
                )
